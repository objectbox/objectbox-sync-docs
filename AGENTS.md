...
- New prose edits shall follow the rule "one sentence per line", including when editing existing sentences;
  this keeps git diffs and history readable.
- For sentences longer than 120 characters, add additional line breaks at other punctuation marks.
- Apply this only to sentences and paragraphs you actually edit; do not reflow otherwise untouched text.
...
no AI attribution lines, no "Generated with" footers, no `Co-Authored-By` trailers.

## File Index

Quick reference of every page in this repo and what it covers.
Keep this index up to date when adding, removing, or renaming pages (just like `SUMMARY.md`).
...
- **`sync-client.md`** — Sync Client docs (large page; prefer reading targeted sections). Covers: obtaining sync-enabled libraries per language (Java/Kotlin, Swift, Dart, C/C++, Go), enabling sync on entity types (`@Sync` annotation), starting a client, sync filter client variables (including IN values), drop-off / send-only clients, sync flags (`RemoveWithObjectData`, debug flags, etc.), authentication options (JWT, shared secret, Google Sign-In, none), manual start, event listeners, and controlling sync update behavior. Multi-language code samples throughout.
- **`mesh-sync.md`** — Mesh Sync (peer-to-peer, currently a preview for Android): syncing clients directly with nearby devices without a central Sync Server connection, initializing via `MeshConfig` on a `SyncClient`, setup, Android permissions, configuration options, diagnostics, current limitations. Multi-language code samples.
- **`syncing-concurrent-changes.md`** — Concurrent updates and conflict resolution: default "last received write wins", sync clock property (hybrid logical clock, HLC) for true last-write-wins, sync precedence property (developer-assigned priority), resolution order when combining both, use-case examples. Multi-language code samples.
...
- **`sync-server/monitoring.md`** — Monitoring and Alerting: Prometheus metrics endpoint (`/api/sync/prometheus` on the admin HTTP server), reference of available `obx_*` metrics (general, clients/connections, messages, transactions, server internals, cluster, MongoDB connector), hooking up Prometheus, Grafana dashboards, alerting rule examples.
...
- **`mongodb-sync-connector/mongodb-statistics.md`** — MongoDB Statistics page in the Admin UI ("MongoDB Connector" → "Statistics"): chart controls (second/minute/hour/day, auto-update, retention), data flow charts (changes from MongoDB, applied to MongoDB, skipped changes/echoes, transactions behind, throughput bytes, bulk operations), detail/error charts (ID mappings, puts skipped on conflict, failed change batches, errors, warnings/retries), what to watch for, pointer to Prometheus monitoring.
- **`mongodb-sync-connector/mongodb-data-mapping.md`** — Data Mapping: type/property name mapping (`@ExternalName`), ID mapping (automatic, special ID types: ObjectID, UUID variants, String, Binary, Int), property/field type mapping tables (standard types, special types like Decimal128, JavaScript, JsonToNative, MongoId, UUID, MongoBinary, MongoRegex, MongoTimestamp), to-one relations, many-to-many relations, nested documents (flex properties vs. JSON string with `@ExternalType(JsonToNative)`), heterogeneous arrays. Multi-language code samples (Java, Kotlin, Swift, Dart, C/C++, Go).
