---
title: "Configuration - ObjectBox C++"
description: "To setup ObjectBox Sync Server to your needs, there are various configuration options, which are presented on this page."
keywords:
  - objectbox
  - c++
  - cpp
...
<TechnicalArticleSchema
  headline="Configuration - ObjectBox C++"
  description="To setup ObjectBox Sync Server to your needs, there are various configuration options, which are presented on this page."
  url="https://cpp.objectbox.io/configuration"
...
Note that both approaches can be [combined](configuration#combining-cli-and-file-configuration).
...
Alternatively, you can choose to provide the configuration in a JSON file. This is the preferred choice if the options are getting more complex (e.g. you can check in the configuration file into version control). Also, it's the only way to configure a [cluster](sync-cluster).
...
* `auth.jwt` JWT is the primary method for authentication. See the [JWT authentication page](jwt-authentication) for details.
...
## Clusters
To set up a cluster, please refer to the [cluster](sync-cluster) page for specific configuration options.
...
For production usage, please refer to the [JWT authentication](./jwt-authentication) page on how to authenticate your clients.
