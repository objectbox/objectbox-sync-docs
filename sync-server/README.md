...
To start the server, you need to pass your data model, which describes the structure of your data. It is a JSON file generated by one of the [ObjectBox Sync clients](../sync-client.md) and is required to initialize the ObjectBox Server database.
...
Before starting the sync server, you typically copy the model file into the directory in which you run the sync server.
...
Once the image is pulled, you can [run](https://docs.docker.com/reference/cli/docker/container/run/) the container. The following example starts Sync Server using the current directory as the data folder and exposes the sync-server on localhost:9999 and the Admin web UI on [http://localhost:9980](http://localhost:9980). Ensure that the file objectbox-model.json is in the current directory on your host (it's mapped to /data inside the container):
