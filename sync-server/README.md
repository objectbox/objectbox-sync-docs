...
To test the Sync Server with your data, ObjectBox offers a free trial. It comes as a publicly available Docker image, which you can pull from GitHub:

```shell
docker pull objectboxio/sync-server-trial
```

Read on how to start it with a model file. Once the Docker is started successfully, you consent to the trial terms in the Admin web interface and you are good to sync.
...
To specify an authentication method for clients provide a [configuration file](./#configuration).
